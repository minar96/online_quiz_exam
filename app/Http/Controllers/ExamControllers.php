...
class ExamControllers extends Controller
{
    public function create(){
    	return view('backend.pages.exam.assign');
    }
    public function assignExam(Request $request){
     ...
     public function userExam(Request $request){
     	$quizzes = Quiz::get();
     	return view('backend.pages.exam.index', compact('quizzes'));
     }
     public function removeExam(Request $request){
     	...
     	if ($result) {
     		return redirect()->back()->with('error', 'This Exam already given so you cannot deleted!!');
     	}else{
     ...
     public function getQuizQuestions(Request $request, $quizId){
        ...
        //check if user has been assigned particular quizz
        ...
        //check logged in user played paritcular quiz
        ...
        if (in_array($quizId, $wasCompleted)) {
            return redirect()->to('/home')->with('error', 'you already completed the exam');
        }
     ...
     public function viewResult($userId, $quizId){
        $results = Result::where('user_id', $userId)
                        ...
                        ->get();
        return view('fontend.result_details', compact('results'));
     }

     public function result()
     {
        $quizzes = Quiz::get();
        return view('backend.pages.result.index', compact('quizzes'));
     }
     public function userQuizResult($userId, $quizId){
        ...
        foreach ($results as $answer) {
            array_push($ans, $answer->answer->answer_id);
        }
        ...
        return view('backend.pages.result.result', compact('results', 'totalQuestion', 'attemptQuestion', 'userCorrectedAnswer', 'userWrongAnswer', 'percentage','quiz'));
     }
